      ...
      <ul class="right_center ">
        <li class="right_center_item" v-for="(item, i) in list" :key="i">
          ...
          <div class="inner_right">
            ...
            <div class="flex">
              <div class="info">
                <span class="labels">标题：</span>
                <span class="contents" style="color: #E6A23C;"> {{ item.title}}</span>
              </div>
            </div>
            <div class="flex">
              <div class="info">
                <span class="labels">链接：</span>
                <a class="contents" target="_blank" :href="item.href">{{ item.href }}</a>
              </div>
            </div>
            <div class="flex">
              <div class="info">
                <span class="labels">时间：</span>
                <span class="contents"> {{ item.post_time}}</span>
              </div>
            </div>
          </div>
        </li>
...
export default {
  ...
  methods: {
    getData() {
      this.pageflag = true
      // this.pageflag =false
      GETNOBASE('http://82.156.0.244:59687/zhengce').then(res => {

        this.list = res.zhengce
