...
module.exports = {
	...
	css: {
		...
		extract: process.env.NODE_ENV === "production" ? true :
		false, //是否将组件中的 CSS 提取至一个独立的 CSS 文件中 (而不是动态注入到 JavaScript 中的 inline 代码)。
	...
	devServer: {
		proxy: {
			...
			'/api': { // 匹配所有以'/api' 开头的请求路径
				target: 'http://82.156.0.244:59687', // 代理目标的基础路径
				changeOrigin: true,
				pathRewrite: { '^/api': '' }
			},
			'/tianqi': {
			...
			'/unicloud': {
				target: 'https://fc-mp-8e18ce11-87b3-420d-9662-c5b9bfd2d497.next.bspapp.com', // 代理目标的基础路径
				changeOrigin: true,
				pathRewrite: { '^/unicloud': '' }
			},
		}
